...
function index_clickCheckAll(el) {
    var className = $(el).children().eq(0).attr("class");
    var liList = $(el).parent().next().children("li");
    if (className.indexOf("checkbox0") != -1) {
        $(el).children().eq(0).addClass("tree-checkbox1");
        $(el).children().eq(0).removeClass("tree-checkbox0");
        index_checkAllOrNotCheckAll(liList,".tree-checkbox0")
    } else {
        $(el).children().eq(0).addClass("tree-checkbox0");
        $(el).children().eq(0).removeClass("tree-checkbox1");
        index_checkAllOrNotCheckAll(liList,".tree-checkbox1")
    }
}

// 点击全选触发事件
function index_clickCheckOther(el) {
    var className = $(el).children().eq(0).attr("class");
    var liList = $(el).parent().next().children("li");
    $(el).prev().children().eq(0).addClass("tree-checkbox0");
    $(el).prev().children().eq(0).removeClass("tree-checkbox1");
    index_checkAllOrNotCheckAll(liList,".tree-checkbox")
}
...
function index_addMousedownDiv(el, id) {
    ...
    if ($(el).children().eq(0).children().eq(0).children(".tree-checkbox").length != 0) {
        ...
        // <span class="tree-checkbox tree-checkbox0" onclick="index_clickCheckOther(this)"><span class="">反选</span></span>
        }
        $(el).parent().prepend('<div class="index_myCheckAll"><span onclick="index_clickCheckAll(this)" style="cursor: pointer"><span class="tree-checkbox tree-checkbox0"  style="top: 1px" ></span><span style="bottom: 3px">全选</span></span>' +
            '<span onclick="index_clickCheckOther(this)" style="cursor: pointer"><span class="tree-checkbox tree-checkbox0"  style="left:10px;top: 1px" ></span><span style="bottom: 3px;left:10px;">反选</span></span></div>')
    }
    // 判断是否已经给该下拉框添加过三角
